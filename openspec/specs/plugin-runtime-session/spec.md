...
invalid acknowledgement, `messageerror`, Host reload, or loss of trusted
identity MUST terminate the current Session; and disposal MUST be idempotent
and clean up the Session's nonce, Ports, and listeners. `disconnected` MUST be
terminal, and the system MUST NOT automatically reauthenticate or reuse an old
Port.
...
- **THEN** the existing container still reports only `loaded`, and the Session
  remains `awaiting_handshake`
- **THEN** UI, logs, state, and documentation do not call it Session ready, SDK
  ready, or Host API available

#### Scenario: Session authentication completes

- **WHEN** an awaiting Session receives its only valid acknowledgement
- **THEN** the Session enters `ready` without creating an SDK Runtime context,
  RPC method, or Host API capability

#### Scenario: Host reload or Port error

- **WHEN** the Host JavaScript realm reloads, the Port emits `messageerror`, or
  the current Session can no longer prove its identity
- **THEN** the old Session reaches terminal disconnect or disposal, and a new
  realm cannot restore its old nonce or Port
- **THEN** a new document and iframe must establish a new Session with a new
  nonce, Port, and current facts, without automatically reconnecting the old
  Session

#### Scenario: Repeated cleanup or late event

- **WHEN** close, retry, invalidation, and App teardown race to dispose, then an
  old acknowledgement or Port event arrives
- **THEN** resources are safely cleaned up once and the Session remains
  `disposed`
- **THEN** the late event changes no current iframe, Session, or Registration
  state
