...
The management page MUST execute local installation, enable, disable,
replacement, and uninstall operations through root-private typed services.
Every entry mutation MUST bind the current opaque entry ID and expected
Registration revision. At most one mutation MAY be active on a management page
at a time, and pending state MUST prevent duplicate submission. After success,
the service MUST wait for the returned revision to converge with the shared
snapshot. A conflict MUST refresh state and require a new user confirmation;
the service MUST NOT replay a destructive action automatically.
...
- **THEN** the page displays the source and target versions, classification,
  and added and removed permission IDs before requiring explicit confirmation
- **THEN** added permissions remain ungranted and the page does not call a grant
  mutation

#### Scenario: A write encounters a revision conflict

- **WHEN** the target Registration revision changes after user confirmation
- **THEN** the service rejects the stale request and the page closes the stale
  confirmation, refreshes complete state, and displays localized conflict feedback
...
The Task 6.1 management page MUST display only current requested, supported,
granted, and effective permission states. It MUST NOT provide grant, revoke,
default-selection, or permission-risk confirmation controls. The page MUST map
only closed error and diagnostic codes to localized actionable feedback and
MUST NOT display a raw error, stack, absolute path, digest, Store key, damaged
record, storage key or value, Tauri or Rust object, or plugin payload.

#### Scenario: A requested permission is not granted

- **WHEN** the Manifest requests a supported permission absent from the grant
  snapshot
- **THEN** detail displays it as requested and not granted rather than usable
  or trusted
- **THEN** the page provides no authorization control and leaves authorization
  interaction to a later permission-prompt capability

#### Scenario: A Host operation returns a safe error

- **WHEN** a typed service returns a validated stable error code
- **THEN** the page displays actionable feedback in the current locale and
  permits an appropriate retry or refresh
- **THEN** raw native messages, paths, stacks, and unvalidated payloads do not
  enter the DOM, log evidence, or accessibility announcement
...
the Semi Design locale and theme, and the fixed Host page surface. List
selection, installation, retry, enable, disable, replacement, uninstall, data
clear, confirmation, and cancellation MUST be operable with only a keyboard,
with visible focus and accessible names. Pending, success, error,
compatibility, and enabled state MUST use text and semantics and MUST NOT rely
on color alone.

#### Scenario: Cancel a destructive confirmation with the keyboard

- **WHEN** a keyboard user opens and cancels an uninstall or clear-data
  confirmation for the selected plugin
- **THEN** the modal has an accessible title, description, and explicit
  destructive and cancel actions
...
The management contract, adapters, services, view model, and data-clear command
MUST exist only in the Rust Host, private Tauri boundary, and trusted root
application. They MUST NOT be exported through `@lensx/plugin-contract`,
...
Rust and TypeScript wire drift, public package and workspace boundaries,
service orchestration, UI state, i18n, theme, keyboard, focus, and visual
...
- **WHEN** an official plugin, example plugin, or external tarball consumer
  attempts to import the management service, data-clear contract, desktop
  adapter, or Tauri command types
- **THEN** the workspace and public-package boundary gate rejects the dependency
- **THEN** plugin code cannot list other plugins, change lifecycle, clear data,
  or read diagnostics

#### Scenario: Run the focused delivery gate

- **WHEN** a maintainer runs the focused plugin-management-settings validation
  command
- **THEN** contract fixtures, Rust and TypeScript tests, service and UI tests,
  boundary checks, bilingual light and dark keyboard states, fixed-viewport
  screenshots, and computed styles pass
