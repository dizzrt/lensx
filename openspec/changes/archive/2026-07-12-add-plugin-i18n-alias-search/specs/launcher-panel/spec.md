...
启动器搜索栏 MUST 作为 launcher 固有内部入口搜索已安装插件暴露的 actions。系统 MUST 从插件 registry 中读取可用 actions，并将每个搜索结果映射到可执行的插件 action。搜索匹配范围 MUST 包含插件 action 标题、插件 action ID、插件 ID、插件英文名、当前应用语言下的插件名称以及该插件所有有效别名。搜索能力 MUST NOT 作为内建搜索插件注册。
