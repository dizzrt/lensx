...
| 维度 | 当前基线 | 含义 |
| --- | --- | --- |
| 公共 package | `0.1.0` | Package exports 与实现 release。 |
| Manifest protocol | `0.1.0` | 作者输入与 normalized Manifest wire contract。 |
| Host API protocol | `0.1.0` | Semantic method、result、event、error 与 context。 |
...
range。SDK 支持 Host API `>=0.1.0 <0.2.0`，并在 ready 前拒绝 incompatible context。
...
不要记录 package content、所选 path、permission reason、stored value 或私有 failure detail。
...
7. 对正式 package 使用 Settings 本地安装，并遵循 Host 受限 preparation/permission result。
8. 在 Runtime 检查最新 context，区分 missing capability、permission denial、unavailable provider、
