...
// This warning already exists in the locked registry release and becomes dependency output only
...
"#,
  ...
  #[cfg(target_os = "macos")]
  if let Some(navigation_handler) = pending.navigation_handler_with_frame {
    webview_builder = webview_builder.with_navigation_handler_with_frame(move |url, frame| {
      let frame = match frame {
        wry::NavigationFrame::Main => tauri_runtime::webview::NavigationFrame::Main,
        wry::NavigationFrame::Descendant => tauri_runtime::webview::NavigationFrame::Descendant,
        wry::NavigationFrame::Unknown => tauri_runtime::webview::NavigationFrame::Unknown,
      };
      url
        .parse()
        .map(|url| navigation_handler(&url, frame))
        .unwrap_or(false)
    });
  }
