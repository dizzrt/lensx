...
#[cfg(target_os = "macos")]
type NavigationHandlerWithFrame = dyn Fn(&Url, NavigationFrame) -> bool + Send;

/// Native target frame classification for a macOS document navigation.
#[cfg(target_os = "macos")]
...
pub struct PendingWebview<T: UserEvent, R: Runtime<T>> {
  ...
  /// A macOS handler that receives the native target frame classification.
  #[cfg(target_os = "macos")]
  pub navigation_handler_with_frame: Option<Box<NavigationHandlerWithFrame>>,
