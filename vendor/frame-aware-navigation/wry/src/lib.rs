...
// These warnings already exist in the locked registry release and become dependency output only
// because lensX applies the reviewed crate as a local path patch.
...
/// Native target frame classification for a document navigation.
#[cfg(any(target_os = "macos", target_os = "ios"))]
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum NavigationFrame {
  /// The top-level document frame.
  Main,
  /// A descendant document frame.
  Descendant,
  /// The native callback did not provide a target frame.
  Unknown,
}
...
struct WebViewAttributes<'a> {
  ...
  /// A navigation handler that also receives the native target frame classification.
  #[cfg(any(target_os = "macos", target_os = "ios"))]
  pub navigation_handler_with_frame: Option<Box<dyn Fn(String, NavigationFrame) -> bool>>,
...
impl<'a> WebViewBuilder<'a> {
  ...
  /// Set a navigation handler that also receives the native target frame classification.
  #[cfg(any(target_os = "macos", target_os = "ios"))]
  pub fn with_navigation_handler_with_frame(
    mut self,
    callback: impl Fn(String, NavigationFrame) -> bool + 'static,
  ) -> Self {
    self.attrs.navigation_handler_with_frame = Some(Box::new(callback));
    self
  }
