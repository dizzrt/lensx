...
use crate::{
  ...
  NavigationFrame, PageLoadEvent, WryWebView,
};
...
pub struct WryNavigationDelegateIvars {
  ...
  pub navigation_policy_function: Box<dyn Fn(String, NavigationFrame) -> bool>,
...
impl WryNavigationDelegate {
  ...
  pub fn new(
    ...
    navigation_handler_with_frame: Option<Box<dyn Fn(String, NavigationFrame) -> bool>>,
    ...
    let navigation_policy_function = Box::new(move |url: String, frame: NavigationFrame| -> bool {
      let legacy_allowed = navigation_handler
        .as_ref()
        .map_or(true, |navigation_handler| (navigation_handler)(url.clone()));
      let frame_allowed = navigation_handler_with_frame
        .as_ref()
        .map_or(true, |navigation_handler| navigation_handler(url, frame));
      legacy_allowed && frame_allowed
    });
