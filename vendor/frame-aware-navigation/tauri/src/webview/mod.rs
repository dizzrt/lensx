...
#[cfg(target_os = "macos")]
pub use tauri_runtime::webview::NavigationFrame;
...
#[cfg(target_os = "macos")]
pub(crate) type NavigationHandlerWithFrame = dyn Fn(&Url, NavigationFrame) -> bool + Send;
...
unstable_struct!(
  ...
  struct WebviewBuilder<R: Runtime> {
    ...
    #[cfg(target_os = "macos")]
    pub(crate) navigation_handler_with_frame: Option<Box<NavigationHandlerWithFrame>>,
...
```
  ...
  /// Defines a macOS closure that receives the native target frame classification before commit.
  #[cfg(target_os = "macos")]
  pub fn on_navigation_with_frame<
    F: Fn(&Url, NavigationFrame) -> bool + Send + 'static,
  >(
    mut self,
    f: F,
  ) -> Self {
    self.navigation_handler_with_frame.replace(Box::new(f));
    self
  }
...
```
  ...
  pub(crate) fn into_pending_webview<M: Manager<R>>(
    ...
    #[cfg(target_os = "macos")]
    {
      pending.navigation_handler_with_frame = self.navigation_handler_with_frame.take();
    }
