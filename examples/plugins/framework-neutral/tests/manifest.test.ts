...
describe('framework-neutral template Manifest', () => {
  test('is accepted by the real Contract with one permission-free Page and matching Action', () => {
    ...
    expect(normalized.manifest.contributes.pages).toEqual([
      expect.objectContaining({ id: 'main', route: '/', required_permissions: [] }),
    ]);
